...
export default function ConfirmedScreen() {
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      ...
      setConfirmedOrders(updatedOrders)
        // 🔄 Gọi lại fetchOrders để cập nhật danh sách đơn hàng ngay sau khi hủy
        fetchOrders();
    } catch (error: any) {
      const errorMessage =
  ...
  // ✅ Hàm fetchOrders để lấy danh sách đơn hàng
  ...
  }, [selectedStatus]); // 🔥 useCallback để tránh re-create hàm không cần thiết

  // 🔄 Gọi fetchOrders() khi component mount hoặc khi selectedStatus thay đổi
  useEffect(() => {
    fetchOrders();
  }, [fetchOrders]);
